...
// Write Protect
//@@@1#define QSPI_FLASH_WRITE_PROTECT_SUPPORT_N25Q

// Status Check Timeout
#define QSPI_FLASH_STATUS_COUNT_N25Q	(10000)		//10s

// Bulk Erase Status Check Timeout
#define QSPI_FLASH_BULK_STATUS_COUNT_N25Q (250000)	//250s

// Bank Size
#define QSPI_FLASH_BANK_SIZE_N25Q		(16*1024*1024)

// Page Size
#define QSPI_FLASH_PAGE_SIZE_N25Q		(256)
...
#define QSPI_WRITE_CMD_N25Q				(0x02)
...
#define QUAD_QSPI_READ_CMD_N25Q			(0x6B)
#define QUAD_QSPI_WRITE_CMD_N25Q		(0x32)
#define QSPI_BULK_ERASE_CMD_N25Q		(0xC7)
#define	QSPI_SEC_ERASE_CMD_N25Q			(0xD8)
...
// Write Protect
#define QSPI_STATUS_TOP_BOTTOM_N25Q		(1<<5)		// Top or Bottom
#define QSPI_STATUS_PROTECT3_N25Q		(1<<6)		// bit3
#define QSPI_STATUS_PROTECT2_N25Q		(1<<4)		// bit2
#define QSPI_STATUS_PROTECT1_N25Q		(1<<3)		// bit1
#define QSPI_STATUS_PROTECT0_SHIFT_N25Q	(1<<2)		// bit0
#define QSPI_STATUS_PROTECT012_SHIFT_N25Q	(2)
#define QSPI_STATUS_DATA3_N25Q			(1<<3)
#define QSPI_STATUS_DATA012_N25Q		(0x07)
#define QSPI_STATUS_MASK_N25Q			(QSPI_STATUS_TOP_BOTTOM_N25Q | QSPI_STATUS_PROTECT3_N25Q | QSPI_STATUS_PROTECT2_N25Q | QSPI_STATUS_PROTECT1_N25Q | QSPI_STATUS_PROTECT0_SHIFT_N25Q)

// Sector Min/Max
#define QSPI_SECTOR_MIN_N25Q			(0)
#define QSPI_SECTOR_MAX_N25Q			(512)
#define QSPI_TOP_BOTTOM_SECTOR_N25Q		(256)
...
int qspiFlashInitialize_N25Q (void)
{
	...
	// Write Protect
	//------------------------------------------------------------
#ifdef QSPI_FLASH_WRITE_PROTECT_SUPPORT_N25Q
	if ((status = qspiFlashWriteProtect_N25Q (-1)) != AVAL_STATUS_SUCCESS)
		goto _DONE;
#endif
...
int qspiFlashWrite_N25Q (unsigned int adrs, unsigned char *pBuffer, unsigned int size)
{
	...
	for (transed=0; transed<size; transed+=transSize, adrs+=transSize)
	{
		...
		{
			if ((status = qspiFlashWriteProtect_N25Q (adrs)) != AVAL_STATUS_SUCCESS)
				goto _DONE;
		}
		#endif
		...
		if (transSize > QSPI_FLASH_PAGE_SIZE_N25Q)
			transSize = QSPI_FLASH_PAGE_SIZE_N25Q;

		//------------------------------------------------------------
		// スタートアドレスの確認
		//------------------------------------------------------------
		if (adrs & (QSPI_FLASH_PAGE_SIZE_N25Q-1))
		{
			// QSPI_FLASH_PAGE_SIZE_N25Qアライン分のサイズのみを転送する
			alignAdrs = adrs & (QSPI_FLASH_PAGE_SIZE_N25Q-1);
			alignSize = QSPI_FLASH_PAGE_SIZE_N25Q - alignAdrs;
		...
		// Writeコマンド発行
		//------------------------------------------------------------
		QspiFlashWriteBuffer[0] = QSPI_WRITE_CMD_N25Q;				// Write Command
		//QspiFlashWriteBuffer[0] = QUAD_QSPI_WRITE_CMD_N25Q;		// Quad Write Command
...
int qspiFlashSectorErase_N25Q (unsigned int adrs)
{
	...
	// セクタイレーズコマンド発行
	//------------------------------------------------------------
	cmdBuff[0] = QSPI_SEC_ERASE_CMD_N25Q;
...
int qspiFlashEraseAll_N25Q (void)
{
	...
	{
		if ((status = qspiFlashSectorErase_N25Q (adrs)) != AVAL_STATUS_SUCCESS)
			goto _DONE;
	}
...
int qspiFlashWriteProtectMake_N25Q (int top, int data, int *pMakeData)
{
	...
	if (top == 0)
		...
		makeData = QSPI_STATUS_TOP_BOTTOM_N25Q;

	// Bit3
	if (data & QSPI_STATUS_DATA3_N25Q)
		makeData |= QSPI_STATUS_PROTECT3_N25Q;

	// Bit012
	makeData |= ((data & QSPI_STATUS_DATA012_N25Q) << QSPI_STATUS_PROTECT012_SHIFT_N25Q);
...
int qspiFlashWriteProtect_N25Q (unsigned int address)
{
	...
	unsigned int ix;
#endif
	u8 ReadStatusCmd[] = { QSPI_READ_STATUS_CMD_N25Q, 0 };
	u8 WriteStatusCmd[] = { QSPI_WRITE_STATUS_CMD_N25Q, 0 };
	int makeData;
	...
	qspiFlashWriteProtectMake_N25Q (top, writeProtectData, &makeData);
	
	// Write Protectデータ設定
	FlashStatus[1] &= ~QSPI_STATUS_MASK_N25Q;
...
int qspiFlashWriteProtectRead_N25Q (int *pStatus)
{
	...
	for (loop=0; loop<2; loop++)
	{
		...
		//------------------------------------------------------------
		if (loop==0)
			WriteStatusCmd[1] = FlashStatus[1] | QSPI_STATUS_TOP_BOTTOM_N25Q;
		else
			WriteStatusCmd[1] = FlashStatus[1] & ~QSPI_STATUS_TOP_BOTTOM_N25Q;
		...
		if (FlashStatus[1] & QSPI_STATUS_PROTECT0_SHIFT_N25Q)
			data |= (1<<0);

		if (FlashStatus[1] & QSPI_STATUS_PROTECT1_N25Q)
			data |= (1<<1);

		if (FlashStatus[1] & QSPI_STATUS_PROTECT2_N25Q)
			data |= (1<<2);

		if (FlashStatus[1] & QSPI_STATUS_PROTECT3_N25Q)
			data |= (1<<3);
