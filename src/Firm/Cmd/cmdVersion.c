...
#if defined (MODE_CXP)
		// CXP FPGA バージョン取得
		if ((status = cxpGetFpgaVersion (&version)) != AVAL_STATUS_SUCCESS)
			goto _DONE;

		DEBUG_PRINT_FORCE ("%x.%x ", version>>4, version&0x0f);
#endif
	}
	else
	{
...
int cmdIfFpgaVersionHelp (void *str)
{
	...
	DEBUG_PRINT_FORCE ("  Function          : Interface FPGA version is acquired.\n");
	DEBUG_PRINT_FORCE ("  Command           : ifversion [param0]\n");
	DEBUG_PRINT_FORCE ("  Input  Param0     : none\n");
	DEBUG_PRINT_FORCE ("  Output Param      : Version\n");
	DEBUG_PRINT_FORCE ("\n");
